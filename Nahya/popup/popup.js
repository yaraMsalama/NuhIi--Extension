...
class NuhyiExtension {
    constructor() {
        ...
        this.settings = {
            ...
            snoozeMinutes: 5 // New setting for snooze duration
        };
    ...
    async init() {
        ...
        chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
            if (message.action === 'showPrayerReminderPopup') {
                this.showPrayerReminderPopup(message.prayerName, message.timeRemaining);
            }
        });
    }
    ...
    setupEventListeners() {
        ...
        // Snooze minutes setting
        document.getElementById('snoozeMinutes').addEventListener('change', (e) => {
            this.settings.snoozeMinutes = parseInt(e.target.value) || 5;
            this.saveSettings();
        });
        ...
        // Test page buttons
        document.getElementById('openTestExtension').addEventListener('click', () => {
            this.openTestPage('test-extension.html');
        });

        document.getElementById('openTestPrayerPopup').addEventListener('click', () => {
            this.openTestPage('test-prayer-popup.html');
        });
    }
    ...
    switchTab(tabName) {
        ...
        switch (tabName) {
            ...
            case 'reminders':
                ...
                break;
            case 'tests':
                // Tests tab doesn't need to load any specific data
                break;
        }
    }
    ...
    // New function to show prayer reminder popup
    showPrayerReminderPopup(prayerName, timeRemaining) {
        ...
        popup.innerHTML = `
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-80 max-w-md border border-blue-200 dark:border-gray-700">
                <div class="p-6 text-center">
                    ...
                    <h3 class="text-lg font-semibold text-blue-800 dark:text-white mb-2">تذكيرة الصلاة</h3>
                    <p class="text-gray-600 dark:text-gray-300 mb-4">
                        فاضل <span class="font-bold text-blue-600">${timeRemaining}</span> دقيقة على أذان صلاة <span class="font-bold text-blue-600">${arabicPrayerName}</span>
                    </p>
                    <div class="flex space-x-3 space-x-reverse">
                        <button id="snoozeBtn" class="flex-1 px-4 py-2 bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
                            تأجيل (${this.settings.snoozeMinutes || 5} د)
                        </button>
                        <button id="okBtn" class="flex-1 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors">
                            موافق
                        </button>
                    </div>
                </div>
    ...
    handleSnooze(prayerName) {
        const snoozeMinutes = this.settings.snoozeMinutes || 5;
        // Schedule another reminder in snooze minutes
        setTimeout(() => {
            this.showPrayerReminderPopup(prayerName, snoozeMinutes.toString());
        }, snoozeMinutes * 60 * 1000);
    }

    // Update pre-prayer alarm handling
    async handlePrePrayerAlarm(prayerName) {
        const result = await chrome.storage.sync.get(['settings']);
        const settings = result.settings || {};
        const prePrayerMinutes = settings.prePrayerMinutes || 10;
        
        // Show popup instead of just notification
        this.showPrayerReminderPopup(prayerName, prePrayerMinutes.toString());
    }
    ...
    openSettings() {
        ...
        document.getElementById('snoozeMinutes').value = this.settings.snoozeMinutes;
